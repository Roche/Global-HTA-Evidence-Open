#' Calculate the survivor functions estimated in a 1st order fractional polynomial NMA model. The absolute S(t) estimates combining the estimated baseline survival
...
get_fp_1o_S <- function(fit, ref.std, ref.arm, treatments,
  ...
  if (length(exponents) != 1){
    out <- NULL
    warning("Fractional polynomial must be of 1st order")
  }
